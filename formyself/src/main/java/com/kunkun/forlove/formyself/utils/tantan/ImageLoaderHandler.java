...
public class ImageLoaderHandler {
    ...
    public static ImageLoaderHandler get() {  // 获取实例
        if (sInstance == null) {
    ...
    public void loadCardImage(Activity activity, ImageView iv, final View loadingView,
                              final String url, final boolean isShowLoadWhenStarted) {
        // 上下文    加载的控件(这里是cardimageview)    loadingview     图片的加载的url
        Glide.with(activity)
                .load(url)
                .dontAnimate()
